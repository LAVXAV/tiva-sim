...
autoatualização, simulação de washout e previsão de recobro.
"""
...
def estimate_ce_curve_schnider(patient: Patient, schedule: List[ScheduleStep], total_duration_min: int, resolution_sec: float = 1.0, stop_min: float = None) -> List[float]:
    ...
    for i in range(1, steps):
        ...
        # Taxa de infusão zero após despertar
        ...
        # Equações PK 2-compartimentos
        dC1 = (dose_mg - Cl1*C1 - Cl2*(C1 - C2)) / V1
        dC2 = (Cl2*(C1 - C2)) / V2
        C1 += dC1 * dt; C2 += dC2 * dt

        # Compartimento efeito
        Ce_prev = Ce[-1]
...
if st.session_state.page == "setup":
    ...
    with col2:
        ...
        equipo = st.selectbox("Equipo", list(EQUIPOS.keys()))
    ...
    st_autorefresh(interval=1000, key="tiva_refresh")
    ...
    for i, ce in enumerate(ce_vals):
        ...
        if wake_time and t>=wake_time: gtt=0
    ...
    if wake_time:
        ...
        if rec: st.success(f"Ce<1 µg/mL em {rec} min pós-despertar.")
        else:    st.warning("Ce ainda >1 µg/mL; continue observação.")
